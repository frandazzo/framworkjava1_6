...
:OkAFHome
if exist "%APPLICAFRAMEWORK_HOME%\target\scripts-%APPLICAFRAMEWORK_VERSION%.jar" goto init
...
:endInit
SET AF_JAR="%APPLICAFRAMEWORK_HOME%\target\lib\*;%APPLICAFRAMEWORK_HOME%\target\scripts-%APPLICAFRAMEWORK_VERSION%.jar"
...
@REM -- Regular WinNT shell
for %%i in ("%APPLICAFRAMEWORK_HOME%"\target\lib\*.jar) do ( set CLASSWORLDS_JAR=!CLASSWORLDS_JAR!;"%%i")
goto runm2
